...
impl FetchData {
    ...
    /// bdfs patch:失败当前水合传输(等价于 CfExecute TRANSFER_DATA + 错误状态)。
    /// 上游 0.0.6 只支持从 [SyncFilter::fetch_data] 返回 Err 由 proxy 同步发送,
    /// 实测该路径会被平台拒绝并杀死宿主进程,见 PATCHES.md;由使用方在回调外调用。
    pub fn fail(&self, error_kind: CloudErrorKind, required: Range<u64>) -> core::Result<()> {
        command::Write::fail_range(self.connection_key, self.transfer_key, error_kind, required)
    }
}
