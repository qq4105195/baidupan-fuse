...
impl Write<'_> {
    /// bdfs patch:带 required range 的失败应答。实测全 0 的 Offset/Length 会被平台
    /// 拒绝(ERROR_CLOUD_OPERATION_INVALID),填成 FETCH_DATA 请求的必需区间。
    pub fn fail_range(
        ...
        execute::<Self>(
            CF_OPERATION_PARAMETERS_0 {
                TransferData: CF_OPERATION_PARAMETERS_0_6 {
                    ...
                    Buffer: ptr::null(),
                    Offset: required.start as i64,
                    Length: required.end.saturating_sub(required.start) as i64,
                },
            },
            connection_key,
            transfer_key,
        ...
        execute::<Self>(
            CF_OPERATION_PARAMETERS_0 {
                TransferData: CF_OPERATION_PARAMETERS_0_6 {
                    Flags: CloudFilters::CF_OPERATION_TRANSFER_DATA_FLAG_NONE,
                    CompletionStatus: error_kind.into(),
                    // bdfs patch: 失败路径必须传空指针。原实现传 [0;1] 指针 + Length=0,
                    // CfExecute 会拒绝(ERROR_CLOUD_OPERATION_INVALID"云操作无效")。
                    // MS 官方 CfApiSample 失败路径即 Buffer=nullptr。
                    Buffer: ptr::null(),
                    Offset: 0,
                    Length: 0,
                },
